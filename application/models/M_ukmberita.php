...
class M_ukmberita extends CI_Model
{
    ...
    public function getSeputarUKMTerbaru()
    {
        $berita = $this->db->from('ukmberita')->where('kodeprodi', 'tif')->order_by('tanggal', 'desc')->limit('4')->get()->result();
        $beritaTerbaru = '<h3 class="judul"><i class="fa fa-newspaper-o" aria-hidden="true"></i> Seputar UKM Terbaru</h3>';
        foreach ($berita as $ber) {
            $image = base_url() . 'assets/gambarDB/berita/ukm/' . $ber->thumbnail;
            $url = base_url() . 'berita-ukm/' . $ber->id;
