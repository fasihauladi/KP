        ...
        <div class="box box-success">
            <div class="box-header">
                ...
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/dosen/tambah"><i class="fa fa-plus"></i> Tambah Dosen</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                <form class="form-row mt-3" id="formFilter">
                    ...
                    <div class="form-group col-12">
                        <select class="form-control bg-primary text-light" name="bidangminat_filter" id="bidangminat_filter" onchange="changeFilter()">
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus Dosen</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusDosen" enctype="multipart/form-data">
                    <!-- nip -->
...
<script>
    ...
    //variabel datatable
    var tabelDosen = $('#tabelDosen');
    $(document).ready(function() {
        var fixedTable = tabelDosen.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "order": [],
            "ajax": {
                "url": "<?= base_url('dosen/readAjaxDosen'); ?>",
                "type": "POST",
                "data": function(data) {
                    data.prodi_filter = $('#prodi_filter').val();
                    data.bidangminat_filter = $('#bidangminat_filter').val();
    ...
    function changeFilter() {
        tabelDosen.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(nip) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('dosen/getdosenbynip/'); ?>' + nip,
            dataType: 'JSON',
            success: function(response) {
                $('#detailPenelitian').html(response.penelitian);
                if (response.foto == null || response.foto == "") {
                    $("#detailFoto").html("<img src='<?= base_url(); ?>assets/images/avatar.png' class='w-25'>");
                } else {
                    $("#detailFoto").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/dosen/" + response.foto + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/dosen/" + response.foto + "' class='w-25' style='width:600px'></a>");
                }
    ...
    function hapusDosenProcess() {
        ...
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>dosen/hapusDataProcess",
            data: $("#formHapusDosen").serialize() + "&csrf_test_name=" + csrfHash,
