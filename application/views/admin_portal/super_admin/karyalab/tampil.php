        ...
        <div class="box box-success">
            <div class="box-header">
                <h3 class="box-title text-capitalize">
                    Data Karya Laboratorium
                </h3>
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/karya-laboratorium/tambah"><i class="fa fa-plus"></i> Tambah Karya Laboratorium</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                ...
                <!-- tabel -->
                <table id="tabelKaryaLaboratorium" class="display" style="width:100%">
                    <thead>
                        <tr>
                            <th class="text-capitalize">No.</th>
                            <th class="text-capitalize">Karya</th>
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus Karya Laboratorium</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusKaryaLaboratorium" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-12" id="inputan_namakarya">
                        ...
                        <input type="text" class="form-control" id="namakarya" name="namakarya" disabled>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" name="tombolHapus" onclick="hapusKaryaLaboratoriumProcess()">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    //variabel datatable
    var tabelKaryaLaboratorium = $('#tabelKaryaLaboratorium');
    $(document).ready(function() {
        var fixedTable = tabelKaryaLaboratorium.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "order": [],
            "ajax": {
                "url": "<?= base_url('karyalab/readAjaxKaryaLaboratorium'); ?>",
                "type": "POST",
                "data": function(data) {
    ...
    function changeFilter() {
        tabelKaryaLaboratorium.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('karyalab/getkaryalabbyid/'); ?>' + id,
            dataType: 'JSON',
            success: function(response) {
                if (response.foto == null || response.foto == "") {
                    $("#detailFoto").html("<img src='<?= base_url(); ?>assets/images/avatar.png' class='w-25'>");
                } else {
                    $("#detailFoto").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/karya/lab/" + response.foto + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/karya/lab/" + response.foto + "' class='w-25' style='width:600px'></a>");
                }
                $('#detailDeskripsi').html(response.deskripsikarya);
    ...
    function hapusKaryaLaboratoriumProcess() {
        $('[name="tombolHapus"]').attr("disabled", true);
        $('[name="tombolHapus"]').text("loading..");
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>karyalab/hapusDataProcess",
            data: $("#formHapusKaryaLaboratorium").serialize() + "&csrf_test_name=" + csrfHash,
