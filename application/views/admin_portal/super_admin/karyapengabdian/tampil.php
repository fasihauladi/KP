        ...
        <div class="box box-success">
            <div class="box-header">
                <h3 class="box-title text-capitalize">
                    Data Karya Pengabdian
                </h3>
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/karya-pengabdian/tambah"><i class="fa fa-plus"></i> Tambah Karya Pengabdian</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                <form class="form-row mt-3" id="formFilter">
                    ...
                    <div class="form-group col-12">
                        <select class="form-control bg-primary text-light" name="kategoripengabdian_filter" id="kategoripengabdian_filter" onchange="changeFilter()">
                ...
                <!-- tabel -->
                <table id="tabelKaryaPengabdian" class="display" style="width:100%">
                    <thead>
                        <tr>
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus Karya Pengabdian</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusKaryaPengabdian" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-12" id="inputan_katpengabdianid">
                        ...
                        <div>
                            <select id="katpengabdianid" name="katpengabdianid" class="form-control bg-info text-light" disabled>
                                <option value="">-</option>
                                <?php foreach ($listKategoriPengabdian as $lkp) : ?>
                                    <option value="<?= $lkp->id; ?>">(<?= $lkp->kategori; ?>) <?= $lkp->subkategori; ?></option>
                                <?php endforeach; ?>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" name="tombolHapus" onclick="hapusKaryaPengabdianProcess()">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    //variabel datatable
    var tabelKaryaPengabdian = $('#tabelKaryaPengabdian');
    $(document).ready(function() {
        var fixedTable = tabelKaryaPengabdian.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "order": [],
            "ajax": {
                "url": "<?= base_url('karyapengabdian/readAjaxKaryaPengabdian'); ?>",
                "type": "POST",
                "data": function(data) {
                    data.prodi_filter = $('#prodi_filter').val();
                    data.kategoripengabdian_filter = $('#kategoripengabdian_filter').val();
    ...
    function changeFilter() {
        tabelKaryaPengabdian.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('karyapengabdian/getkaryapengabdianbyid/'); ?>' + id,
            dataType: 'JSON',
            success: function(response) {
                if (response.foto == null || response.foto == "") {
                    $("#detailFoto").html("<img src='<?= base_url(); ?>assets/images/avatar.png' class='w-25'>");
                } else {
                    $("#detailFoto").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/karya/pengabdian/" + response.foto + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/karya/pengabdian/" + response.foto + "' class='w-25' style='width:600px'></a>");
                }
    ...
    function hapusKaryaPengabdianProcess() {
        $('[name="tombolHapus"]').attr("disabled", true);
        $('[name="tombolHapus"]').text("loading..");
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>karyapengabdian/hapusDataProcess",
            data: $("#formHapusKaryaPengabdian").serialize() + "&csrf_test_name=" + csrfHash,
