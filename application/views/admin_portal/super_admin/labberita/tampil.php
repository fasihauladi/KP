        ...
        <div class="box box-success">
            <div class="box-header">
                <h3 class="box-title text-capitalize">
                    Data Seputar Laboratorium
                </h3>
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/seputar-laboratorium/tambah"><i class="fa fa-plus"></i> Tambah Seputar Laboratorium</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                <form class="form-row mt-3" id="formFilter">
                    ...
                    <div class="form-group col-12">
                        <select class="form-control bg-primary text-light" name="laboratorium_filter" id="laboratorium_filter" onchange="changeFilter()">
                ...
                <!-- tabel -->
                <table id="tabelLabBerita" class="display" style="width:100%">
                    <thead>
                        <tr>
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus Seputar Laboratorium</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusLabBerita" enctype="multipart/form-data">
                    <input type="hidden" name="id" id="id">
                    <!-- judulberita -->
                    <div class="form-group col-12" id="inputan_judulberita">
                        <label for="judulberita" class="form-control-placeholder">Judul</label>
                        <input type="text" class="form-control" id="judulberita" name="judulberita" disabled>
                    ...
                    <div class="form-group col-12" id="inputan_kodelab">
                        ...
                        <div>
                            <select id="kodelab" name="kodelab" class="form-control bg-info text-light" disabled>
                                <option value=""> --- Pilih Laboratorium --- </option>
                                <?php foreach ($listLaboratorium as $ll) : ?>
                                    <option value="<?= $ll->kodelab; ?>"><?= $ll->namalab; ?></option>
                                <?php endforeach; ?>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" name="tombolHapus" onclick="hapusDosenProcess()">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    //variabel datatable
    var tabelLabBerita = $('#tabelLabBerita');
    $(document).ready(function() {
        var fixedTable = tabelLabBerita.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "order": [],
            "ajax": {
                "url": "<?= base_url('labberita/readAjaxLabBerita'); ?>",
                "type": "POST",
                "data": function(data) {
                    data.prodi_filter = $('#prodi_filter').val();
                    data.laboratorium_filter = $('#laboratorium_filter').val();
    ...
    function changeFilter() {
        tabelLabBerita.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('labberita/getlabberitabyid/'); ?>' + id,
            dataType: 'JSON',
            success: function(response) {
                if (response.thumbnail == null || response.thumbnail == "") {
                    ...
                    $("#detailThumbnail").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/berita/lab/" + response.thumbnail + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/berita/lab/" + response.thumbnail + "' class='w-25' style='width:600px'></a>");
                }
                if (response.foto == null || response.foto == "") {
                    $("#detailFoto").html("<img src='<?= base_url(); ?>assets/images/avatar.png' class='w-25'>");
                } else {
                    $("#detailFoto").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/berita/lab/" + response.foto + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/berita/lab/" + response.foto + "' class='w-25' style='width:600px'></a>");
                }
    ...
    function hapusDosenProcess() {
        $('[name="tombolHapus"]').attr("disabled", true);
        $('[name="tombolHapus"]').text("loading..");
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>labberita/hapusDataProcess",
            data: $("#formHapusLabBerita").serialize() + "&csrf_test_name=" + csrfHash,
