...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo base_url('superadmin/seputar-pengabdian/edit/' . $id) ?>"> Edit</a></li>
        </ol>
    </section>
    <!-- main content -->
        ...
        <div class="box box-success">
            <div class="box-header">
                <h3 class="box-title text-capitalize">
                    Edit Seputar Pengabdian
                </h3>
            </div>
            <div class="box-body">
                <form class="form-row" id="formEditPengabdianBerita" enctype="multipart/form-data">
                ...
                <div id="btnEdit">
                    <button type="button" class="btn btn-primary" name="tombolEdit" onclick="editPengabdianBeritaProcess()">Edit</button>
                </div>
            </div>
        </div>
    </section>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url('pengabdianberita/getpengabdianberitabyid/'); ?>' + $('#id').val(),
            dataType: 'JSON',
            success: function(response) {
                ...
                if (response.thumbnail) {
                    $('#tempatThumbnail').html('<img height="200" width="200" class="rounded-circle" src="<?= base_url(); ?>assets/gambarDB/berita/pengabdian/' + response.thumbnail + '" alt="Thumbnail" id="lihatThumbnail" name="lihatThumbnail">');
                }
                if (response.foto) {
                    $('#tempatFoto').html('<img height="200" width="200" class="rounded-circle" src="<?= base_url(); ?>assets/gambarDB/berita/pengabdian/' + response.foto + '" alt="Foto" id="lihatFoto" name="lihatFoto">');
                }
    ...
    function editPengabdianBeritaProcess() {
        ckeditor();
        $('[name="tombolEdit"]').attr("disabled", true);
        $('[name="tombolEdit"]').text("loading..");
        let formData = new FormData($('#formEditPengabdianBerita').get(0));
        formData.append("csrf_test_name", csrfHash);
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>pengabdianberita/editDataProcess",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
