        ...
        <div class="box box-success">
            <div class="box-header">
                <h3 class="box-title text-capitalize">
                    Data Peta Penelitian
                </h3>
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/peta-penelitian/tambah"><i class="fa fa-plus"></i> Tambah Peta Penelitian</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                ...
                <!-- tabel -->
                <table id="tabelPetaPenelitian" class="display" style="width:100%">
                    <thead>
                        <tr>
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus Peta Penelitian</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusPetaPenelitian" enctype="multipart/form-data">
                    <input type="hidden" id="id" name="id">
                    <!-- namadok -->
                    <div class="form-group col-12" id="inputan_namadok">
                        <label for="namadok" class="form-control-placeholder">Nama Dokumen</label>
                        <input type="text" class="form-control" id="namadok" name="namadok" disabled>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" name="tombolHapus" onclick="hapusPetaProcess()">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    //variabel datatable
    var tabelPetaPenelitian = $('#tabelPetaPenelitian');
    $(document).ready(function() {
        var fixedTable = tabelPetaPenelitian.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
            "order": [],
            "ajax": {
                "url": "<?= base_url('peta/readAjaxPetaPenelitian'); ?>",
                "type": "POST",
                "data": function(data) {
    ...
    function changeFilter() {
        tabelPetaPenelitian.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('peta/getpetabyid/'); ?>' + id,
            dataType: 'JSON',
    ...
    function hapusPetaProcess() {
        $('[name="tombolHapus"]').attr("disabled", true);
        $('[name="tombolHapus"]').text("loading..");
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>peta/hapusDataProcess",
            data: $("#formHapusPetaPenelitian").serialize() + "&csrf_test_name=" + csrfHash,
