...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="<?php echo base_url('superadmin/seputar-prodi/edit/' . $id) ?>"> Edit</a></li>
        </ol>
    </section>
    <!-- main content -->
    <section class="content">
        <div class="box box-success">
            ...
            <div class="box-body">
                <form class="form-row" id="formEditProdiBerita" enctype="multipart/form-data">
                ...
                <div id="btnEdit">
                    <button type="button" class="btn btn-primary" name="tombolEdit" onclick="editProdiBeritaProcess()">Edit</button>
                </div>
            </div>
        </div>
    </section>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url('prodiberita/getprodiberitabyid/'); ?>' + $('#id').val(),
            dataType: 'JSON',
            success: function(response) {
                ...
                if (response.foto) {
                    $('#tempatFoto').html('<img height="200" width="200" class="rounded-circle" src="<?= base_url(); ?>assets/gambarDB/berita/prodi/' + response.foto + '" alt="Foto" id="lihatFoto" name="lihatFoto">');
                }
    ...
    function editProdiBeritaProcess() {
        ckeditor();
        $('[name="tombolEdit"]').attr("disabled", true);
        $('[name="tombolEdit"]').text("loading..");
        let formData = new FormData($('#formEditProdiBerita').get(0));
        formData.append("csrf_test_name", csrfHash);
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>prodiberita/editDataProcess",
            data: formData,
            processData: false,
            contentType: false,
            cache: false,
            ...
            success: function(data) {
                ...
                if (data.message == 'sukses') {
                    Swal.fire('Data Berhasil Diedit', '', 'success').then(function() {
                        window.location = "<?= base_url(); ?>superadmin/seputar-prodi";
                    });
