        ...
        <div class="box box-success">
            <div class="box-header">
                ...
                <div class="box-tools pull-right">
                    <a class="btn btn-success btn-flat" role="button" href="<?= base_url(); ?>superadmin/unit-kegiatan-mahasiswa/tambah"><i class="fa fa-plus"></i> Tambah UKM</a>
                </div>
            </div>
            <div class="box-body">
                <!-- filter tabel -->
                ...
                <!-- tabel -->
                <table id="tabelUKM" class="display" style="width:100%">
                    <thead>
                        <tr>
...
<div class="modal fade" id="hapusModal" tabindex="-1" role="dialog" aria-labelledby="hapusModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title" id="hapusModalLabel">Hapus UKM</h4>
            </div>
            <div class="modal-body">
                <form class="form-row" id="formHapusUKM" enctype="multipart/form-data">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" name="tombolHapus" onclick="hapusUKMProcess()">Hapus</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    //variabel datatable
    var tabelUKM = $('#tabelUKM');
    $(document).ready(function() {
        var fixedTable = tabelUKM.DataTable({
            "responsive": true,
            "processing": true,
            "serverSide": true,
            "stateSave": true,
    ...
    function changeFilter() {
        tabelUKM.DataTable().ajax.reload(null, false);
    }

    function bukaModalDetail(id) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('ukm/getukmbyid/'); ?>' + id,
            dataType: 'JSON',
            success: function(response) {
                if (response.foto == null || response.foto == "") {
                    $("#detailFoto").html("<img src='<?= base_url(); ?>assets/images/avatar.png' class='w-25'>");
                } else {
                    $("#detailFoto").html("<a class='example-image-link' target='_blank' href='<?= base_url(); ?>assets/gambarDB/ukm/" + response.foto + "' data-lightbox='example-1'><img src='<?= base_url(); ?>assets/gambarDB/ukm/" + response.foto + "' class='w-25' style='width:600px'></a>");
                }
    ...
    function hapusUKMProcess() {
        $('[name="tombolHapus"]').attr("disabled", true);
        $('[name="tombolHapus"]').text("loading..");
        $.ajax({
            type: "POST",
            url: "<?= base_url(); ?>ukm/hapusDataProcess",
            data: $("#formHapusUKM").serialize() + "&csrf_test_name=" + csrfHash,
