<?php
$ci = &get_instance();
$ci->load->model('M_karyapenelitian', 'karyapenelitian');
?>
<section id="event">
	<div class="container ">
		<div>
			<div class="event_header text-center">
				<h2><?= ucwords($kategoriPenelitiannya->namakatpen); ?></h2>
			</div>
			<div id="exTab1" class="container">
				<ul class="nav nav-pills">
					...
					<?php foreach ($subKategori as $sk) : ?>
						<li class="<?= $aktifnya ?>">
							<a href="<?= $ahrefnya; ?>" data-toggle="tab" style="text-transform:capitalize"><?= $sk->namasubkatpen; ?></a>
						</li>
						<?php
						$aktifnya = '';
						$ahrefnya = $ahrefnya . 'a';
									...
									<div class="row" style="margin:7px">
										<table id="<?= $tabelnya ?>" class="table table-striped table-bordered" style="width:100%;">
											...
											<tbody>
												<?php
												$nomorUrut = 1;
												$dataKaryaPenelitian = $ci->karyapenelitian->getAllDataByKategoriPenelitianId($sk->id);
												?>
												<?php foreach ($dataKaryaPenelitian as $dkp) : ?>
													<tr>
														<td class="text-center"><?= $nomorUrut; ?></td>
														<td><a href="<?= base_url(); ?>karya-penelitian/<?= $dkp->id; ?>"><?= $dkp->judul; ?></a></td>
													</tr>
													<?php $nomorUrut += 1; ?>
												<?php endforeach; ?>
											</tbody>
